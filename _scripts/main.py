...
def main(base_args, args_train, args_attack):    
    ...
    method = "baseline" if base_args.baseline else "diff_pruning"
    if args_train.bitfit: method = "bitfit"
    logger_name = "_".join([
        method,
    ...
    if not base_args.baseline:
        trainer_cls = DiffNetwork
    else:
        if args_train.adverserial:
            trainer_cls = AdverserialNetwork
        else:
            trainer_cls = TaskNetwork
